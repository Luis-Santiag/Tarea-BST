...
public class ArbolBinarioBusqueda {
    ...
    /**
     * Cuenta todos los nodos del arbol usando recursividad.
     * No utiliza el campo tamanio.
     */
    ...
    /**
     * Un arbol esta balanceado cuando, en cada nodo, la diferencia
     * de alturas entre sus subarboles izquierdo y derecho es como maximo 1.
     */
    public boolean esBalanceado() {
        return alturaSiBalanceado(raiz) != -2;
    }

    private int alturaSiBalanceado(Nodo nodo) {
        if (nodo == null) {
            return -1;
        }

        int alturaIzquierda = alturaSiBalanceado(nodo.izquierdo);
        if (alturaIzquierda == -2) {
            return -2;
        }

        int alturaDerecha = alturaSiBalanceado(nodo.derecho);
        if (alturaDerecha == -2) {
            return -2;
        }
    ...
    /**
     * Verifica que todos los nodos cumplan la propiedad del BST.
     */
    public boolean esBSTValido() {
        return esBSTValidoRecursivo(raiz, null, null);
    }

    private boolean esBSTValidoRecursivo(Nodo nodo, Integer minimoPermitido, Integer maximoPermitido) {
        if (nodo == null) {
            return true;
        }

        if (minimoPermitido != null && nodo.dato <= minimoPermitido.intValue()) {
            return false;
        }
        if (maximoPermitido != null && nodo.dato >= maximoPermitido.intValue()) {
            return false;
        }

        return esBSTValidoRecursivo(nodo.izquierdo, minimoPermitido, Integer.valueOf(nodo.dato))
                && esBSTValidoRecursivo(nodo.derecho, Integer.valueOf(nodo.dato), maximoPermitido);
    }

    /**
     * Devuelve el ancestro comun mas bajo entre dos valores existentes.
     */
    ...
    /**
     * Invierte el arbol completo, intercambiando izquierdo y derecho
     * en todos los nodos.
     */
